...
export function ShelfOverlay({ paths, repoColorFor, onRestore }: ShelfOverlayProps) {
  if (paths.length === 0) return null;
  return (
    <div className="shelf">
      <span className="shelf-label">SHELF</span>
      {paths.map((p) => (
        <ShelfChip key={p} path={p} colorIndex={repoColorFor(p)} onRestore={onRestore} />
      ))}
    </div>
  );
}
...
function ShelfChip({ path, colorIndex, onRestore }: ShelfChipProps) {
  ...
  return (
    <div
      ...
      onPointerUp={(e) => {
        ...
        onRestore(path, moved.current ? { clientX: e.clientX, clientY: e.clientY } : undefined);
        moved.current = false;
      }}
    >
