...
describe("S77 — kitty flag 8 switches the whole delivery path", () => {
  ...
  it("reports the UNSHIFTED physical key, which is what kitty's CSI u encodes", () => {
    ...
    const rows: Array<[string, string, number, boolean]> = [
      ...
      ["1", "Digit1", 49, false],
      ["9", "Digit9", 57, false],
      // The shifted digit row, every face of it.
      [")", "Digit0", 48, true],
      ["!", "Digit1", 49, true],
      ["@", "Digit2", 50, true],
      ["#", "Digit3", 51, true],
      ...
      ["*", "Digit8", 56, true],
      ["(", "Digit9", 57, true],
      // Every punctuation key, both faces — a wrong keyCode on any one of them
      // sends a program a key nobody pressed.
      [" ", "Space", 32, false],
      [";", "Semicolon", 186, false],
      [":", "Semicolon", 186, true],
      ["=", "Equal", 187, false],
      ...
      ["'", "Quote", 222, false],
      ['"', "Quote", 222, true],
      // No layout to guess from, so nothing is reported — the same rule as S24.
      ["中", "", 0, false],
    ];
